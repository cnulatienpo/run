...
function openDB() {
    ...
    dbPromise = new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = () => {
            ...
            const stores = ['profiles', 'decks', 'items', 'mnemonics', 'plans', 'sessions', 'prefs'];
...
export async function cacheMedia(request, response) {
...
async function cryptoKeyFromPassphrase(passphrase, salt) {
    const enc = new TextEncoder();
    const baseKey = await crypto.subtle.importKey('raw', enc.encode(passphrase), 'PBKDF2', false, ['deriveKey']);
    const saltBuffer = salt.slice().buffer;
    return crypto.subtle.deriveKey({ name: 'PBKDF2', salt: saltBuffer, iterations: 75000, hash: 'SHA-256' }, baseKey, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']);
}
export async function exportPack(passphrase) {
...
export async function importPack(file, passphrase) {
