...
# Use -a so xvfb-run auto-selects a free display if :99 is already in use.
exec xvfb-run -a --server-args='-screen 0 1280x720x24' -- npx electron --no-sandbox --disable-gpu .
