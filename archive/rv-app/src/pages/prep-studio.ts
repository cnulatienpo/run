...
class RVPrepStudio extends HTMLElement {
  ...
  private uploadInput = document.createElement('input');
  ...
  private render() {
    ...
    this.uploadInput.addEventListener('change', (event) => {
      ...
      if (file) this.controller.ingestFile(file);
    });
  ...
  private renderInterview() {
    ...
    this.interviewForm.addEventListener('submit', (event) => {
      ...
      const devices = (data.getAll('devices') as string[]).map((value) => value as any);
      this.controller.saveProfile({
        challenge: data.get('challenge') as string,
        devices: devices.length ? (devices as any) : ['pun'],
        absurdity: data.get('absurdity') as any,
        audioMode: data.get('audio') as any,
        talkback: data.get('talkback') as any,
  ...
  private renderPreview() {
    ...
    generate.addEventListener('click', () => {
      ...
      if (deck) this.controller.generateMnemonics(deck);
    });
  ...
  private populateMnemonics(container: HTMLElement) {
    container.innerHTML = '';
